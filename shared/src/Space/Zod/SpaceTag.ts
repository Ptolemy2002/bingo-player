...
export const ZodSpaceTagSchema = swaggerRegistry.register(
    ...
    z.string()
        ...
        .regex(/^(in\:)?[a-zA-Z0-9-][a-zA-Z0-9_-]+$/, {
            error: "Invalid tag. Must be alphanumeric (including dashes and underscores) and start with a letter, number, or dash. May start with 'in:' to indicate collection membership."
        })
        .toLowerCase()
        .openapi({
